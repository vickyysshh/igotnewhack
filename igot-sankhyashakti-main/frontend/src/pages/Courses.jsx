...
const rows=(searchResults||data.courses).filter(c=>(skill==='All'||c.competency===skill)&&(difficulty==='All'||c.difficulty===difficulty)&&(provider==='All'||c.provider.includes(provider))&&(duration==='All'||(duration==='short'?c.duration<=6:c.duration>6))&&(department==='All'||c.department===department));
const reset=()=>{setSkill('All');setDifficulty('All');setProvider('All');setDuration('All');setDepartment('All');setQuery('');setParams({});setPage(1);};
return <div className="page-enter">
  <PageHead eyebrow="LEARNING ALIGNED WITH YOUR COMPETENCIES" title="iGOT Course Explorer" description="Discover relevant learning. Close meaningful competency gaps." action={<Badge tone="orange" testId="igot-demo-integration"><Info size={13}/> Learning Integration</Badge>}/>
  <div className="filter-tabs course-tabs"><button className={tab==='courses'?'active':''} data-testid="courses-tab" onClick={()=>setParams({})}>iGOT Courses <span>{data.courses.length}</span></button><button className={tab==='programmes'?'active':''} data-testid="programmes-tab" onClick={()=>setParams({tab:'programmes'})}>NSSTA / TPAC Programmes <span>{data.training_programmes.length}</span></button></div>
  {tab==='programmes'?<>
    <div className="programmes-intro"><Building2 size={32}/><div><h2>Build expertise through focused training.</h2><p>Faculty-led programmes recommended for your role and statistical domain.</p></div></div>
    <div className="programme-grid">{data.training_programmes.map(p=><article className="programme-card" key={p.id} data-testid={`programme-${p.id}`}><Badge tone="blue" testId={`programme-provider-${p.id}`}>{p.provider}</Badge><h2>{p.title}</h2><p>Recommended to strengthen {p.competency.toLowerCase()} capabilities aligned with your current role.</p><div className="programme-meta"><span><CalendarDays size={16}/>{p.date}</span><span><Clock size={16}/>{p.duration}</span><span><MapPin size={16}/>{p.location}</span></div><div className="programme-bottom"><span>{p.seats} seats</span><Btn disabled={p.registered||busy===p.id} data-testid={`register-${p.id}`} onClick={async()=>{setBusy(p.id);try{await act(()=>api.post(`/programmes/${p.id}/register`),'Interest registered for this programme.');}catch{}finally{setBusy('');}}}>{p.registered?<><CheckCircle2 size={15}/> Interest Registered</>:busy===p.id?'Registering…':<>Register Interest <ArrowRight size={15}/></>}</Btn></div></article>)}</div>
  </>:<>
    <div className="semantic-search"><Sparkles size={22}/><input placeholder='Try “training for AI in official statistics”' value={query} onChange={e=>setQuery(e.target.value)} data-testid="course-search" aria-label="Search courses semantically"/><span className="semantic-badge">Semantic-style search</span><Search size={21}/></div>
    <div className="course-filters">{[['Competency',skill,setSkill,[...new Set(data.courses.map(c=>c.competency))]],['Difficulty',difficulty,setDifficulty,['Beginner','Intermediate','Advanced']],['Provider',provider,setProvider,['iGOT Karmayogi','NSSTA']],['Department',department,setDepartment,[...new Set(data.courses.map(c=>c.department))]]].map(([label,value,setter,options])=><CourseFilter key={label} label={label} value={value} onChange={value=>{setter(value);setPage(1);}} options={options}/>)}
      <label>Duration<select value={duration} data-testid="course-filter-duration" onChange={e=>{setDuration(e.target.value);setPage(1);}}><option value="All" label="Any duration"/><option value="short" label="Up to 6 hours"/><option value="long" label="More than 6 hours"/></select></label><button className="text-link" data-testid="reset-course-filters" onClick={reset}>Reset filters</button>
    </div>
    <div className="results-line" data-testid="course-results-count">{searching?'Finding relevant courses…':`${rows.length} courses available`}<span>Curated for official statistical roles</span></div>
    {searchError?<div role="alert" data-testid="course-search-error">{searchError}</div>:rows.length?<div className="course-grid">{rows.slice((page-1)*6,page*6).map((c,i)=><CourseCard key={c.id} course={c} index={i}/>)}</div>:<Empty/>}
